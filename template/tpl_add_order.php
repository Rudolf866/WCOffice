      ...
      <form id="add_order_form" method="post" action="/iwaterTest/backend.php">
         ...
         <div class="content_add" style="width: 650px;">
            <div style="width: 50%; display: inline-block;">
               ...
               <label for="client_num">Клиент id:</label>
            ...
            <label for="driver">Водитель</label>
             <select id="driver" name="driver" style="margin: 5px 0 20px 0; max-width: 204px; width: 204px;">
<!--                 <option value="0" selected>--</option>-->
 ...
 <script>
   ...
   var swapClientNumber = () => get == "get_once" ? window.location = "/iwaterTest/admin/add_order/" : window.location = "/iwaterTest/admin/add_order?once=once";
   ...
   function validateForm() {
      ...
      $("#add_order_form").validate({
           rules: {
               client_num: { required: true },
               name: { required: true },
               time_d: { required: true },
               address: { required: true },
               cords: { required: true },
               cash: { },
               cash_b: { },
               on_floor: { dollarsscents: true }
           },
           messages: {
               name: { required: "Заполните поле" },
               time_d: { required: "Выберите значение" },
               address: { required: "Заполните поле" },
               client_num: { required: "Заполните поле" },
               cords: { required: "Заполните поле" },
   ...
   */
   ...
   /** 
    * Загрузка координат для временного адреса
   */
   function check_point() {
    if (!myMap) {
     ...
     var pointCoord;
      ...
      var address = 'Россия ' + $("#region").val() + ' ' + $("#address").val();
   ...
   /* if (!myMap) {
      ...
      $.ajax({
         ...
         success: function (data) {
            ...
            console.log('start draw', gooCoord);
            var image = {
...
function setCoords(coords) {
  $('#cords')[0].value = coords[1] + ',' + coords[0];
  $("label[for='cords']").show();
  $('#cords').show();
