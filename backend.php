...
*/
        ...
        if (isset($_POST['add_client'])) { // Добавить клиента
            ...
            $phone = trim(filter_input(INPUT_POST, 'contact', FILTER_SANITIZE_SPECIAL_CHARS));
            $info = trim(filter_input(INPUT_POST, 'information'));
            ...
            try {
                $res = $dbh->query('SELECT `contact`,REPLACE(`contact`,"+7","8") FROM `iwater_clients` WHERE contact LIKE "%'.$phone.'%" ');
                $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            } catch (Exception $e) {
                echo 'Подключение не удалось: ' . $e->getMessage();
            }
            ...
            if ($contact) {
                ...
                try {
                    $res = $dbh->prepare("INSERT INTO `iwater_clients`(`type`, `name`,`contact`, `company_id`) VALUES (?, ?, ?, ?)");
                    $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                } catch (Exception $e) {
                    echo 'Подключение не удалось: ' . $e->getMessage() . '</br>';
                }
                $res->execute(array($type, $name, $phone, $company));
                $insertId =$dbh->lastInsertId();

//                try {
//                    $res = $dbh->prepare("UPDATE `iwater_clients` SET `client_id` = ? WHERE `id` = ? ");
//                    $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
//                    $res->execute(array($insertId, $insertId));
//                } catch (Exception $e) {
//                    echo 'Подключение не удалось: ' . $e->getMessage();
//                }
                ...
                while ($i < $z) {
                    ...
                    $res->execute(array($insertId, $info, $region, $address, $coords, $region . ', ' . $address));

                    try {
                        $res = $dbh->prepare("UPDATE `iwater_clients` SET `client_id` = ?,`region`=?,`address`=?,`address_full`=?,`coords`=? WHERE `id` = ? ");
                        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        $res->execute(array($insertId,$region,$address,$region . ', ' . $address,$coords, $insertId));
                    } catch (Exception $e) {
                        echo 'Подключение не удалось: ' . $e->getMessage();
                    }
...
*/
        ...
        if (isset($_POST['client_num_l'])) {
            ...
            try {
                $res = $dbh->query("SELECT * FROM `iwater_users` WHERE session = '" . $_SESSION['fggafdfc'] . "'");// Проверяют по сессии кто сидит на странички и его компанию
                $usersess = $res->fetch();
                $company = $usersess['company_id'];

                $dbh->query('SET CHARACTER SET utf8');
                $res = $dbh->query("SELECT DISTINCT c.client_id,c.name,a.address FROM `iwater_clients` AS c LEFT JOIN `iwater_addresses` AS a ON c.client_id=a.client_id WHERE c.company_id = '$company' AND c.client_id LIKE ('$array_digits')");
                $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            } catch (Exception $e) {
                echo 'Подключение не удалось: ' . $e->getMessage();
            }
         ...
         if (isset($_POST['name_l'])) {
             ...
             try {
                 ...
                 $res = $dbh->query("SELECT DISTINCT c.name,c.client_id,a.address FROM `iwater_clients` AS c LEFT JOIN `iwater_addresses` AS a ON c.client_id=a.client_id WHERE  c.company_id = '$company' AND c.name LIKE ('%$name%')");
                 $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
             } catch (Exception $e) {
                 echo 'Подключение не удалось: ' . $e->getMessage();
             }
...
*/
        ...
        if (isset($_POST['edit_client'])) {
            ...
            try {
                $res = $dbh->prepare("UPDATE `iwater_clients` SET `type`=?,`contact`=?, `name`=?, `client_id`=? WHERE id='$id';");
                $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $res->execute(array($type,$phone,$name, $num_c));
            } catch (Exception $e) {
                echo 'Подключение не удалось: ' . $e->getMessage() . '</br>';
            }
        ...
        if (isset($_POST['add_list'])) {
            ...
            */
             ...
             */
             try {
                $driver_l = $dbh->query("SELECT DISTINCT u.name, u.id FROM `iwater_orders` AS o LEFT JOIN `iwater_users` AS u ON(driver = u.id) WHERE `date` = '$date' AND o.company_id = '$company'");
                $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
             } catch (Exception $e) {
                echo 'Подключение не удалось: ' . $e->getMessage();
             }
        ...
        if (isset($_POST['unique'])) {
            ...
            switch ($_POST['unique']) {
                case "contact":
                    try {
                        $res = $dbh->query("SELECT COUNT(`contact`) AS count FROM `iwater_clients` WHERE `contact`='$value'");
                        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    } catch (Exception $e) {
                        echo 'Подключение не удалось: ' . $e->getMessage();
                    }
...
{ //водителю
   ...
   */
   ...
   if ($list != "") {
    ...
    $extraSQL .= " AND u.company_id ='$company' ";
